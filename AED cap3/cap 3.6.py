...
class Array(object):
    ...
    def swap(self, j, k):
        ...
        if(0 <= j and j < self.__nItems and
    ...
    def selectionSort(self):
        for outer in range(self.__nItems-1):
            ...
            min = outer
            for inner in range(outer+1, self._nItems):
    ...
    def remove_duplicates_insertion_sort(arr):
    n = len(array.ArrayType)
    low_key = float('-inf')
    count = 0
    
    for i in range(1, n):
        key = array.ArrayType[i]
        j = i - 1
        while j >= 0 and array.ArrayType[j] > key:
            if array.ArrayType[j] == key:
                array.ArrayType[j] = low_key
                count += 1
            j -= 1
        array.ArrayType[j + 1] = key
    
    for i in range(count):
       array.ArrayType[i] = array.ArrayType[n - count + i]
    
    for i in range(n - count, n):
        array.ArrayType[i] = low_key
